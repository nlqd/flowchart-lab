...
(function (root, factory) {
  ...
  function enforceFlow(nodes, edges, opts) {
    var o = opts || {};
    ...
    for (i = 0; i < nodes.length; i++) if (order.indexOf(i) < 0) order.push(i);

    var pushed = 0, violated = 0;
    ...
    for (i = 0; i < real.length; i++) {
      if (nodes[real[i][1]][axis] <= nodes[real[i][0]][axis] + 0.5) violated++;
    }
  ...
  function layout(graph, opts, deps) {
    ...
    var flowInfo = null;
    if (o.flow === 'TB' || o.flow === 'LR') {
      flowInfo = enforceFlow(nodes, edges, { axis: o.flow === 'TB' ? 'y' : 'x', gap: o.flowGap });
    }
    var overlapInfo = o.removeOverlaps
    ...
    return {
      ...
      stats: {
        ...
        epochs: o.epochs,
        backward: flowInfo ? flowInfo.backwardEdges : null,
